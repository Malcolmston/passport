...
// go-ui Layout, switching which view renders.
export function App() {
  ...
  const brand = { title: 'malcolmston', sub: '/passport', initials: 'pp', href: '#overview' };
  const footer: ReactNode = (
    <div>
      <span className="grad-text" style={{ fontWeight: 700 }}>malcolmston/passport</span> — simple, unobtrusive authentication for Go.
      <div className="small dim" style={{ marginTop: '.4rem' }}>MIT licensed · an independent re-implementation of jaredhanson/passport</div>
    </div>
  );

  return (
    <Layout brand={brand} tabs={TABS} active={active} onNav={go} github={LIB.repo} footer={footer}>
      {active === 'overview' && <Overview lib={LIB} />}
      {active === 'releases' && <ReleasesView />}
      {active === 'docs' && <DocsView lib={LIB} />}
    </Layout>
  );
}
