...
// Activate a nav tab link. Across 100+ device profiles the tab bar renders three
...
test('internal in-page links actually navigate to their target', async ({ page }) => {
  await gotoTab(page, 'overview');
  // The Overview "on this page" nav jumps to the Features section.
  await page.locator('.view.active a[href="#passport-feat"]').first().click();
  await expect(page).toHaveURL(/#passport-feat$/);
  await expect(page.locator('#passport-feat')).toBeVisible();
});
